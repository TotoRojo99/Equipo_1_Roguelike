...
public class E_Controller : MonoBehaviour
{
    ...
    [SerializeField] private MenuDeMejorasController controlMejoras; // 👈 Nueva referencia
    ...
    void NuevaRonda()
    {
        ...
        // 🔹 Notificar al menú de mejoras
        if (controlMejoras != null)
            controlMejoras.NuevaRonda(round);

        {
            controlMejoras.NuevaRonda(round);
            Debug.Log($"[E_Controller] Notificando nueva ronda al menú de mejoras: {round}");
        }
    }
